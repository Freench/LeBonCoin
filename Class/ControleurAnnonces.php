...
class ControleurAnnonces extends Bdd{
    function __construct(){
        if($this->analyseEntreeAnnonce()){ 
            ...
            foreach($donnes as $key => $donne){
                $this->insertAnnoncesDetails(intval($key+1), $donne, $newIdAnnonce);
            }
        }
    ...
    function insertAnnoncesDetails($numOrdre, $valeur, $idAnnonce){
        ...
        $pdo = $this->connect();
        $sql =$pdo ->prepare($requete);
        $sql -> execute([$numOrdre, $valeur, $idAnnonce]);
        return $pdo->lastInsertId();
    }
